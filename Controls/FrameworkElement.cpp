...
void XITEM_FrameworkElement::ApplyProperties()
{
	...
	{
		if (IsSomeParentCanvas(this->the_par))
		{
			if (p->n == L"FlowDirection")
			{
				auto op = std::dynamic_pointer_cast<LIST_PROPERTY>(p);
				if (op)
				{
					e.FlowDirection((winrt::Microsoft::UI::Xaml::FlowDirection)op->SelectedIndex);
				}
			}
			if (p->n == L"Canvas.Left")
			{
				auto op = std::dynamic_pointer_cast<DOUBLE_PROPERTY>(p);
				if (op)
				{
					Canvas::SetLeft(e, op->value);
				}
			}
			if (p->n == L"Canvas.Top")
			{
				...
				if (op)
				{
					Canvas::SetTop(e, op->value);
				}
			}
			if (p->n == L"Canvas.ZIndex")
			{
				auto op = std::dynamic_pointer_cast<DOUBLE_PROPERTY>(p);
				if (op)
				{
					Canvas::SetZIndex(e, (int)op->value);
				}
			}
		}

		if (IsSomeParentGrid(this->the_par))
		{
		...
		if (p->n == L"Tooltip")
		{
			...
			{
				ToolTip tooltip;
				tooltip.Content(box_value(op->value));
...
std::vector<std::shared_ptr<PROPERTY>> XITEM_FrameworkElement::CreateProperties(XML3::XMLElement* el)
{
	...
	if (1)
	{
		...
		op->n = L"FlowDirection ";
		op->Items = { L"LeftToRight", L"RightToLeft" };
		op->DefaultIndex = 0;
		p.push_back(op);
	...
	// Canvas
	auto uip2 = CreatePropertiesForParentCanvas(X);
	for (auto& p1 : uip2)
	{
		p1->g = L"Parent Canvas";
		properties.push_back(p1);
	}
